...
export function buildSalaryFilter(salaryFilter, reporter = console) {
  ...
  if (!Number.isFinite(min) || !Number.isFinite(max) || min < 0 || max < 0) {
    reporter.error?.('Warning: salary_filter.min/max must be non-negative numbers — salary filter disabled');
    return () => true;
  }
  if (max > 0 && min > max) {
    reporter.error?.('Warning: salary_filter.min cannot exceed salary_filter.max — salary filter disabled');
    return () => true;
  }
...
export function loadReApplyWindows(profilePath = 'config/profile.yml', fileSystem = fs) {
  if (!fileSystem.existsSync(profilePath)) return {};
  try {
    const raw = yaml.load(fileSystem.readFileSync(profilePath, 'utf-8')) || {};
...
export function loadSeenUrls(policy = {}, paths = {}, fileSystem = fs) {
  const historyPath = paths.scanHistoryPath ?? SCAN_HISTORY_PATH;
  const pipelinePath = paths.pipelinePath ?? PIPELINE_PATH;
  const applicationsPath = paths.applicationsPath ?? APPLICATIONS_PATH;
  const reportsDir = paths.reportsDir ?? 'reports';
  const seen = new Set();
  let recheckEligible = 0;

  // scan-history.tsv
  if (fileSystem.existsSync(historyPath)) {
    const lines = fileSystem.readFileSync(historyPath, 'utf-8').split('\n');
  ...
  if (fileSystem.existsSync(applicationsPath)) {
    const text = fileSystem.readFileSync(applicationsPath, 'utf-8');
    for (const match of text.matchAll(/https?:\/\/[^\s|)]+/g)) {
      seen.add(normalizeVacancyUrl(match[0]) ?? match[0]);
    }
    for (const match of text.matchAll(/\]\((?:\.\.\/)?reports\/([^)]+\.md)\)/g)) {
      const reportPath = path.join(reportsDir, match[1]);
      if (!fileSystem.existsSync(reportPath)) continue;
      const report = fileSystem.readFileSync(reportPath, 'utf-8');
 ...
 */
export function loadSeenCompanyRoles(appsPath = APPLICATIONS_PATH, canonicalize = defaultCompanyNormalizer, fileSystem = fs) {
  const seen = new Set();
  if (fileSystem.existsSync(appsPath)) {
    ...
    const lines = fileSystem.readFileSync(appsPath, 'utf-8').split('\n');
 ...
 */
export function loadFingerprintHistory(historyPath = SCAN_HISTORY_PATH, fileSystem = fs) {
  if (!fileSystem.existsSync(historyPath)) return [];
  const rows = [];
  for (const line of fileSystem.readFileSync(historyPath, 'utf-8').split('\n')) {
...
export function appendToPipeline(offers, { pipelinePath = PIPELINE_PATH } = {}, fileSystem = fs) {
  if (offers.length === 0) return [];

  // Auto-create with standard skeleton if missing (fresh-install guard).
  if (!fileSystem.existsSync(pipelinePath)) {
    fileSystem.writeFileSync(pipelinePath, PIPELINE_SKELETON, 'utf-8');
  }

  let text = fileSystem.readFileSync(pipelinePath, 'utf-8');
  ...
  fileSystem.writeFileSync(pipelinePath, text, 'utf-8');
  return fresh;
}

export function appendToScanHistory(offers, date, status = 'added', { scanHistoryPath = SCAN_HISTORY_PATH } = {}, fileSystem = fs) {
  ...
  if (!fileSystem.existsSync(scanHistoryPath)) {
    fileSystem.writeFileSync(scanHistoryPath, 'url\tfirst_seen\tportal\ttitle\tcompany\tstatus\tlocation\n', 'utf-8');
  }

  const lines = offers.map(o => formatScanHistoryRow(o, date, status)).join('\n') + '\n';

  fileSystem.appendFileSync(scanHistoryPath, lines, 'utf-8');
}
 ...
 * @returns {Map<string, {company: string, since: string, scope: string, reason: string}>}
 */
export function loadBlacklist(filePath = BLACKLIST_PATH, fileSystem = fs) {
  if (!fileSystem.existsSync(filePath)) return new Map();
  return parseBlacklist(fileSystem.readFileSync(filePath, 'utf-8'));
}
...
export function appendScanRunSummary(c, filePath = SCAN_RUNS_PATH, fileSystem = fs) {
  if (!fileSystem.existsSync(filePath)) fileSystem.writeFileSync(filePath, SCAN_RUNS_HEADER, 'utf-8');
  const row = [
  ...
  fileSystem.appendFileSync(filePath, row, 'utf-8');
}
...
export function appendPortalHealth(healthRecords, filePath = PORTAL_HEALTH_PATH, fileSystem = fs) {
  if (!fileSystem.existsSync(filePath)) fileSystem.writeFileSync(filePath, PORTAL_HEALTH_HEADER, 'utf-8');
  let lines = '';
  for (const r of healthRecords) {
    lines += [r.timestamp, r.company, r.status].join('\t') + '\n';
  }
  if (lines) fileSystem.appendFileSync(filePath, lines, 'utf-8');
}

export function loadPortalHealth(filePath = PORTAL_HEALTH_PATH, fileSystem = fs) {
  if (!fileSystem.existsSync(filePath)) return [];
  const lines = fileSystem.readFileSync(filePath, 'utf-8').split('\n');
  const records = [];
...
async function verifyOffers(offers, { headedFallback = false, throttleBaseMs = 0, rediscover = false, reporter = console } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      if (result === 'expired') {
        ...
        if (rediscover && code === 'http_gone' && offer.tracked && offer.careersUrlDomain) {
          ...
          if (newUrl) {
            ...
            if (recheck.result === 'active') {
              ...
              reporter.log?.(`  🔄 migrated  ${offer.company} | ${offer.title} → ${newUrl}`);
              continue;
            }
          }
        }
        expired.push({ ...offer, reason });
        reporter.log?.(`  ❌ expired   ${offer.company} | ${offer.title} (${reason})`);
        ...
        reporter.log?.(`  ⛔ invalid   ${offer.company} | ${offer.title} (${reason})`);
        ...
        reporter.log?.(`  ⚠️ no-apply  ${offer.company} | ${offer.title} (${reason})`);
      } else {
        // 'active' or 'uncertain' due to navigation_error (transient — retry next scan)
        verified.push(offer);
        const icon = result === 'active' ? '✅' : '⚠️';
        reporter.log?.(`  ${icon} ${result.padEnd(9)} ${offer.company} | ${offer.title}`);
      }
...
/**
 * Execute one portal scan without reading CLI arguments or loading portals.yml.
 *
 * @param {object} inputConfig Parsed portals configuration.
 * @param {object} [adapters]
 * @param {object} [adapters.fs] Node-compatible filesystem adapter.
 * @param {{now: () => Date}} [adapters.clock]
 * @param {{loadProviders?: Function, resolveProvider: Function, mergeProviderPlugins?: Function}} [adapters.providerRegistry]
 * @param {{verifyOffers: Function}} [adapters.liveness]
 * @param {{log?: Function, error?: Function}} [adapters.reporter]
 * @returns {Promise<{date: string, offers: object[], found: number, stats: object, errors: object[], agentHandoff: object[], crossListings: object[], healthRecords: object[], dryRun: boolean}>}
 */
export async function runScan(inputConfig, adapters = {}) {
  const fileSystem = adapters.fs ?? fs;
  const clock = adapters.clock ?? { now: () => new Date() };
  const reporter = adapters.reporter ?? globalThis.console;
  const console = reporter;
  const providerRegistry = adapters.providerRegistry ?? { loadProviders, resolveProvider, mergeProviderPlugins };
  const liveness = adapters.liveness ?? { verifyOffers };
  const paths = adapters.paths ?? {};
  const options = adapters.options ?? {};
  const dryRun = Boolean(options.dryRun);
  const verify = Boolean(options.verify);
  const headedFallback = Boolean(options.headedFallback);
  const throttleBaseMs = options.throttleBaseMs ?? 0;
  const rediscover = Boolean(options.rediscover);
  const includeBlacklisted = Boolean(options.includeBlacklisted);
  const filterCompany = options.company ? String(options.company).toLowerCase() : null;
  const config = inputConfig && typeof inputConfig === 'object' ? inputConfig : {};
  const scanStartedAt = clock.now();
  const date = scanStartedAt.toISOString().slice(0, 10);
  if (!dryRun) fileSystem.mkdirSync?.(paths.dataDir ?? 'data', { recursive: true });
  const providers = adapters.providers ?? await providerRegistry.loadProviders(adapters.providersDir ?? PROVIDERS_DIR);
  ...
  await providerRegistry.mergeProviderPlugins?.(providers, { root: adapters.root ?? path.dirname(PROVIDERS_DIR) });
  if (providers.size === 0) {
  ...
  const postingAgeFilter = buildPostingAgeFilter(config.max_posting_age_days, scanStartedAt.getTime());
  const salaryFilter = buildSalaryFilter(config.salary_filter, reporter);
  ...
  function resolveEntries(entries, { isBoard = false } = {}) {
    for (const entry of entries) {
      ...
      if (typeof entry.name !== 'string' || !entry.name.trim()) {
        reporter.error?.(`⚠️  Skipping entry — missing or non-string 'name' field: ${JSON.stringify(entry)}`);
        continue;
      }
      if (filterCompany && !entry.name.toLowerCase().includes(filterCompany)) continue;

      const resolved = providerRegistry.resolveProvider(entry, providers);
      if (!resolved) {
        skippedCount++;
  ...
  reporter.log?.(`Scanning ${parts.join('; ')} via providers`);
  if (dryRun) reporter.log?.('(dry run — no files will be written)\n');

  // 3.5. Load the user's do-not-apply list (#1742). Opt-in: absent file =
  // empty Map = the filter below never fires.
  const blacklist = loadBlacklist(paths.blacklistPath, fileSystem);

  // 4. Load dedup sets
  const historyPolicy = { ...scanHistoryPolicy(config), today: date };
  const seenUrlState = loadSeenUrls(historyPolicy, paths, fileSystem);
  const seenUrls = seenUrlState.seen;
  const canonicalizeCompany = buildCompanyCanonicalizer(config.company_aliases);
  const applicationsPath = paths.applicationsPath ?? APPLICATIONS_PATH;
  const seenCompanyRoles = loadSeenCompanyRoles(applicationsPath, canonicalizeCompany, fileSystem);

  // 5. Fetch from each target
  const windows = loadReApplyWindows(paths.profilePath, fileSystem);
  ...
  const tasks = targets.map(company => async () => {
    ...
    const ctx = (adapters.createHttpContext ?? makeHttpCtx)();
    ...
    try {
      ...
      try {
        ...
        const fallback = providerRegistry.resolveProvider(company, providers, { skipIds: ['local-parser'] });
  ...
  if (verify && newOffers.length > 0) {
    ...
    const result = await liveness.verifyOffers(newOffers, { headedFallback, throttleBaseMs, rediscover, reporter });
  ...
  const fitScore = adapters.fitScore ?? (await import('./fit-score.mjs')).attachFitScores;
  const scoredOffers = fitScore(verifiedOffers, config, {
    matchedKeywordFor: (o) => matchedTitleKeywords(o.title, config.title_filter)[0],
    fs: fileSystem,
    profilePath: paths.profilePath,
    cvPath: paths.cvPath,
    signalsPath: paths.rankingSignalsPath,
    nowMs: scanStartedAt.getTime(),
  });
  verifiedOffers = scoredOffers.sort((a, b) => (b.fitScore ?? 0) - (a.fitScore ?? 0));
  const crossListings = findCrossListings(verifiedOffers, loadFingerprintHistory(paths.scanHistoryPath, fileSystem));

  // 6. Write results
  if (!dryRun && verifiedOffers.length > 0) {
    appendToPipeline(verifiedOffers, paths, fileSystem);
    appendToScanHistory(verifiedOffers, date, 'added', paths, fileSystem);
  }
  if (!dryRun && cooldownOffers.length > 0) {
    ...
    for (const [status, group] of Object.entries(cooldownGroups)) {
      appendToScanHistory(group, date, status, paths, fileSystem);
    }
  }
  ...
  if (!dryRun && expiredForHistory.length > 0) {
    appendToScanHistory(expiredForHistory, date, 'skipped_expired', paths, fileSystem);
  }
  ...
  if (!dryRun && droppedOffers.length > 0) {
    appendToScanHistory(droppedOffers, date, 'skipped_no_apply_control', paths, fileSystem);
  }
  ...
  const pastHealth = loadPortalHealth(paths.portalHealthPath, fileSystem);
  ...
  if (!dryRun) {
    appendPortalHealth(healthRecords, paths.portalHealthPath, fileSystem);
    appendScanRunSummary({
      timestamp: clock.now().toISOString(), status: 'completed',
  ...
  return {
    date, offers: verifiedOffers, found: totalFound,
    stats: {
      companies: summaryCompanies, boards: summaryBoards, skipped: skippedCount,
      filteredTitle: totalFilteredTitle, filteredTier: totalFilteredTier,
      filteredLocation: totalFilteredLocation, filteredRemoteRelocation: totalFilteredRemoteRelocation,
      filteredPostingAge: totalFilteredPostingAge, filteredSalary: totalFilteredSalary,
      filteredContent: totalFilteredContent, filteredCooldown: totalFilteredCooldown,
      filteredBlacklist: totalFilteredBlacklist, annotatedBlacklisted, dupes: totalDupes,
      expired: expiredOffers.length, dropped: droppedOffers.length,
      invalid: invalidOffers.length, migrated: migratedOffers.length, errors: errors.length,
    },
    errors, agentHandoff, crossListings, healthRecords, dryRun,
  };
}
