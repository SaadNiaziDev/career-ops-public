...
const scripts = [
  ...
  { name: 'test/cv-renderer.test.mjs', expectExit: 0 },
...
// ── 4. DASHBOARD BUILD (removed — UI-first fork uses web/) ───────

console.log('\n4. Dashboard build (removed — web UI is primary)');
pass('Go TUI removed in UI-first fork');
...
// ── 7c. UPDATER DASHBOARD REBUILD (removed) ─────────────────────

console.log('\n7c. Updater dashboard rebuild (removed)');
pass('update-system no longer rebuilds Go dashboard');
...
pass('Locale mode packs removed in UI-first fork (English-only)');
...
// MCP fallback — so the flag actually reaches the JD paths, not just scan/pipeline.
{
  ...
  if (langPipelines.length === 0) {
    pass('language pipeline mirrors removed in UI-first fork (English-only modes)');
...
`, 'utf-8');
  ...
  if (validProviderPluginResult !== null && validProviderPluginResult.includes('0 errors')) {
    pass('validate-portals accepts core provider ids');
  } else {
    fail('validate-portals should accept core provider ids');
  }
...
pass('Legacy GEMINI.md / OPENCODE.md removed in UI-first fork');
...

{
  ...
  try {
    ...
    const cursorDir = join(fixtureRoot, '.cursor', 'skills', 'career-ops');
    ...
    writeFileSync(join(cursorDir, 'SKILL.md'), pointer);

    const skills = await import(pathToFileURL(join(ROOT, 'skill-entrypoints.mjs')).href);
    ...
    const cursorSkill = readFileSync(join(cursorDir, 'SKILL.md'), 'utf-8');
    if (claudeSkill === fixtureSkill && cursorSkill === fixtureSkill) {
...

{
  ...
  try {
    ...
    const cursorSkill = readFileSync(join(fixtureRoot, '.cursor', 'skills', 'career-ops', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
    if (cursorSkill === fixtureSkill && claudeSkill === fixtureSkill) {
...

{
  ...
  try {
    ...
    const sample = [
      "import { a } from './skill-entrypoints.mjs';",
...

{
  ...
  // ONLY update-system.mjs into an otherwise-empty dir and
  ...
  try {
    ...
    try {
      ...
      pass('update-system.mjs imports standalone without sibling modules present (#1706)');
    } catch (err) {
...

{
  ...
  try {
    ...
    const materialized = skills.materializeSkillEntrypoints(fixtureRoot);
    const cursorSkill = readFileSync(join(cursorDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.cursor/skills/career-ops/SKILL.md']) && cursorSkill === fixtureSkill) {
...

{
  ...
  try {
    ...
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.cursor/skills/career-ops/SKILL.md`]);

    const updater = await import(pathToFileURL(join(ROOT, 'update-system.mjs')).href);
    const skills = await import(pathToFileURL(join(ROOT, 'skill-entrypoints.mjs')).href);
    const materialized = skills.materializeSkillEntrypoints(fixtureRoot);
    updater.prepareMaterializedSkillEntrypointsForStage(materialized, fixtureRoot);
    gitRun(['add', '--', '.claude/skills/', '.cursor/skills/']);

    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/career-ops/SKILL.md']);
    const cursorIndex = gitRun(['ls-files', '-s', '--', '.cursor/skills/career-ops/SKILL.md']);
    if (claudeIndex.startsWith('100644 ') && cursorIndex.startsWith('100644 ')) {
      pass('materialized skill entrypoints stage as regular files, not symlink blobs');
    } else {
      fail(`materialized skill entrypoints staged with wrong modes: ${JSON.stringify([claudeIndex, cursorIndex])}`);
    }

    const claudeBlob = gitRaw(['show', ':.claude/skills/career-ops/SKILL.md']);
    const cursorBlob = gitRaw(['show', ':.cursor/skills/career-ops/SKILL.md']);
    if (claudeBlob === fixtureSkill && cursorBlob === fixtureSkill) {
...
try {
  ...
  const removedEvaluators = ['ollama-eval.mjs', 'openai-eval.mjs', 'gemini-eval.mjs', 'openrouter-runner.mjs']
    .filter(name => !fileExists(name));
  if (removedEvaluators.length === 4) {
    pass('legacy headless evaluators removed in UI-first fork');
  } else {
...
// ── 44. openrouter-runner (removed in UI-first fork) ─────────────
console.log('\n44. openrouter-runner (removed)');
pass('OpenRouter runner removed — web UI uses Claude Code or Codex');

// ── 44b. openrouter-runner prompt-cache (removed) ────────────────
console.log('\n44b. openrouter-runner prompt-cache (removed)');
pass('OpenRouter prompt-cache tests skipped');
...
pass('Plugin engine removed in UI-first fork — scan uses core providers only');
